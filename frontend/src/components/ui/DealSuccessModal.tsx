...
export function DealSuccessModal({ open, productName, finalPrice, savings, currency, onClose, shareUrl }: DealSuccessModalProps) {
  ...
  return (
    <div
      ...
      data-testid="deal-success-modal"
      onClick={(e) => {
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="deal-success-title"
        style={{ overscrollBehavior: "contain" }}
        className="animate-pop-in relative w-full max-w-md overflow-hidden rounded-3xl bg-panel p-5 text-ink shadow-lift">
        {/* falling confetti */}
        ...
        <button className="absolute right-3 top-3 z-10 rounded-full bg-panelSoft p-2 transition-colors hover:brightness-95" aria-label="Закрыть" onClick={onClose}>
          <X size={16} />
        </button>
        <div className="relative">
          ...
          <h2 id="deal-success-title" className="mt-4 font-display text-2xl font-black leading-tight">Команда закрыта</h2>
          <p className="mt-2 text-sm font-bold text-inkSoft">{productName}</p>
          <div className="mt-4 overflow-hidden rounded-2xl bg-fire-gradient p-4 text-white shadow-glow">
            ...
            <p className="font-display text-4xl font-black tracking-tight tabular-nums">{formatPrice(finalPrice, currency, locale)}</p>
            <p className="mt-2 inline-flex rounded-lg bg-gold-gradient px-2.5 py-1 text-sm font-black text-charcoal shadow-sm tabular-nums">
              сэкономили {formatPrice(savings, currency, locale)}
            </p>
          </div>
          <div className="mt-4 space-y-2">
            <p className="text-xs font-black uppercase tracking-wide text-inkSoft">Способ оплаты</p>
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: "Kaspi Pay", color: "bg-[#FF6B00]", emoji: "🟠" },
                { label: "Kaspi Red", color: "bg-primary", emoji: "🔴" },
                { label: "Halyk", color: "bg-emerald-600", emoji: "🟢" },
              ].map(({ label, color, emoji }) => (
                <button
                  key={label}
                  type="button"
                  className={`rounded-xl ${color} py-2.5 text-xs font-black text-white transition-[filter] hover:brightness-110 active:scale-[0.97]`}
                >
                  <span aria-hidden="true">{emoji}</span> {label}
                </button>
              ))}
            </div>
            <p className="rounded-xl border border-mint/30 bg-mint/10 px-3 py-2 text-xs font-semibold text-mint">
          ...
          <p className="mt-3 flex items-center justify-center gap-1 text-xs font-bold text-inkSoft">
