    ...
    <div className="mx-auto w-full max-w-2xl pb-28 lg:max-w-5xl">
      ...
      <header className="relative overflow-hidden rounded-b-3xl bg-fire-gradient px-4 py-5 text-white shadow-glow">
        ...
        <div className="relative">
          <div className="flex items-center justify-between">
            <Link
              to="/feed"
              className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-sm font-black text-white/90 ring-1 ring-white/30 backdrop-blur-sm transition-colors hover:bg-white/25"
            >
              <ArrowLeft size={16} />
              Лента
            </Link>
            <button
              data-testid="logout"
              onClick={logout}
              className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-sm font-black text-white/90 ring-1 ring-white/30 backdrop-blur-sm transition-colors hover:bg-white/25"
            >
              <LogOut size={15} />
              Выйти
            </button>
          ...
          <div className="mt-5 flex items-center gap-4">
            <div className="animate-float grid h-16 w-16 shrink-0 place-items-center rounded-full bg-white/20 text-3xl font-black ring-2 ring-white/40 backdrop-blur-sm" aria-hidden="true">
              {user.name.slice(0, 1)}
            </div>
            <div>
      ...
      <section className="mx-4 mt-4 grid grid-cols-3 gap-3">
        {[
          ...
          { icon: TrendingDown, label: "сэкономлено", value: null, display: formatPrice(totalSaved, "KZT"), color: "bg-mint/10 text-mint" },
          { icon: Send, label: "инвайтов", value: inviteSent, color: "bg-blue-50 text-blue-600" },
        ].map(({ icon: Icon, label, value, display, color }, idx) => (
          <div
            key={label}
            className="animate-card-in flex flex-col items-center justify-center rounded-2xl border border-hairline bg-panel py-4 shadow-card"
            style={{ animationDelay: `${idx * 50}ms` }}
          >
            <span className={`grid h-8 w-8 place-items-center rounded-xl ${color}`}>
              <Icon size={17} aria-hidden="true" />
            </span>
            <p className="mt-1.5 text-xl font-black text-ink tabular-nums">
      ...
      <div>
      {/* Streak */}
      <section className="mx-4 mt-3 rounded-2xl border border-hairline bg-panel p-4 shadow-card">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-inkSoft">Заходи каждый день и собирай монеты</p>
            </div>
          </div>
          <span className="font-display text-2xl font-black text-fire tabular-nums">{streakDays}</span>
        </div>
        <div className="mt-3">
          <div className="flex gap-1.5">
            {Array.from({ length: 7 }, (_, i) => (
              <div
                key={i}
                className={`flex-1 rounded-lg py-3 text-center text-[10px] font-black transition-[background-color,box-shadow] ${
                  i < streakDays ? "bg-fire-gradient text-white shadow-glow" : "bg-panelSoft text-inkSoft"
                }`}
              >
      ...
      <section className="mx-4 mt-3 rounded-2xl border border-hairline bg-panel p-4 shadow-card">
        ...
        <div className="mt-3 grid grid-cols-4 gap-2">
          {ACHIEVEMENTS.map(({ icon: Icon, label, done }, idx) => (
            <div
              key={label}
              className={`animate-card-in flex flex-col items-center gap-1 rounded-xl p-3 text-center transition-[background-color,box-shadow] ${
                done ? "bg-primary/10 text-primary ring-1 ring-primary/20" : "bg-panelSoft text-inkSoft"
              }`}
              style={{ animationDelay: `${idx * 50}ms` }}
            >
              <Icon size={22} aria-hidden="true" />
              <p className="text-[10px] font-black leading-tight">{label}</p>
              {done && <span className="text-[10px] font-black text-mint">✓</span>}
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Coins progress */}
      <section className="relative mx-4 mt-3 overflow-hidden rounded-2xl border border-gold/40 bg-gold-gradient p-4 shadow-card">
        <div className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full bg-white/30 blur-2xl" />
        <div className="relative">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Gift size={20} aria-hidden="true" />
              <p className="text-sm font-black text-charcoal">До секретного купона</p>
            </div>
            <span className="text-sm font-black text-charcoal tabular-nums">{coins}/300 монет</span>
          </div>
          <ProgressBar value={(coins / 300) * 100} className="mt-3" />
          <p className="mt-2 text-xs font-bold text-charcoal/80">
            {300 - coins > 0
          ...
          {LEADERBOARD.map((entry, idx) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] font-bold text-inkSoft">{entry.city}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-black text-ink tabular-nums">{entry.invites} инв.</p>
                <p className="text-[11px] font-bold text-mint tabular-nums">
                  {formatPrice(entry.saved, "KZT")}
                </p>
              </div>
              {idx === 0 && <Star size={16} className="shrink-0 text-amber-400" aria-hidden="true" />}
            </div>
          ))}
        </div>
      </section>
        ...
        <section className="mx-4 mt-3 rounded-2xl border border-hairline bg-panel p-4 shadow-card">
          ...
          <div className="mt-3 space-y-2">
            {visibleGroups.slice(0, 4).map((g) => (
              <div key={g.id} className="flex items-center justify-between rounded-xl bg-primary/10 px-3 py-2 ring-1 ring-primary/20">
                <div>
                  <p className="text-xs font-black text-ink">Группа #{g.id}</p>
                  <p className="text-[10px] font-bold text-inkSoft tabular-nums">
