    ...
    <div className="mx-auto w-full max-w-2xl pb-28 lg:max-w-5xl">
      <header className="relative overflow-hidden rounded-b-3xl bg-ink-gradient px-4 py-5 text-white shadow-lift lg:mx-4 lg:mt-4 lg:rounded-3xl">
        ...
        <div className="relative">
          <Link
            to="/feed"
            className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-sm font-black text-white/90 ring-1 ring-white/15 backdrop-blur-sm transition hover:bg-white/20"
          >
            <ArrowLeft size={16} />
            Лента
          </Link>
      ...
      <div className="lg:mx-4 lg:mt-4 lg:grid lg:grid-cols-[0.9fr_1.1fr] lg:items-start lg:gap-4">
        {/* Add product */}
        <section className="mx-4 mt-4 rounded-2xl border border-stone-100 bg-white p-4 shadow-card lg:mx-0 lg:mt-0">
          <div className="flex items-center gap-2">
            <Plus size={18} className="text-primary" />
            <h2 className="text-base font-black text-stone-900">Добавить товар</h2>
          </div>
          <form className="mt-3 space-y-3" onSubmit={addProduct}>
            <input
              required
              data-testid="seller-name"
              className="w-full rounded-xl bg-stone-50 px-3 py-2.5 text-sm text-ink outline-none ring-1 ring-black/5 focus:ring-2 focus:ring-primary/30"
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="space-y-1 text-xs font-bold text-stone-500">
                Розница, $
                <input
                  required
                  type="number"
        ...
        <section className="mx-4 mt-4 rounded-2xl border border-stone-100 bg-white p-4 shadow-card lg:mx-0 lg:mt-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-black text-stone-900">Мои товары</h2>
            </div>
          ...
          {loading ? (
            ...
            <p className="py-8 text-center text-sm font-medium text-stone-400">
            ...
            <div className="mt-3 space-y-2">
              {products.map((p) => (
                <div
                  key={p.id}
                  className="flex items-center gap-3 rounded-xl border border-stone-100 bg-stone-50/60 p-2.5"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="line-clamp-1 text-sm font-bold text-stone-900">{p.name}</p>
                  ...
                  <button
                    ...
                    onClick={() => remove(p.id)}
                    className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-stone-400 ring-1 ring-black/5 transition hover:text-primary active:scale-95"
                    aria-label="Удалить"
                  >
                    <Trash2 size={16} />
                  </button>
