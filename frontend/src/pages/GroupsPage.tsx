...
export function GroupsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl bg-appBg pb-28">
      <header className="glass sticky top-0 z-30 border-b border-hairline px-4 py-4 md:px-8">
        <h1 className="font-display text-2xl font-black text-ink">Групповые покупки</h1>
      ...
      <section className="mx-4 mt-4 flex flex-wrap items-center gap-2 md:mx-8">
        <ArrowDownUp size={16} className="text-inkSoft" aria-hidden="true" />
        {[
          ["savings", "лучшая цена"],
          ["time", "горят таймеры"],
          ["category", "категория"],
          ...
          <button
            ...
            className={`rounded-full px-3.5 py-1.5 text-sm font-semibold transition-[background-color,box-shadow,transform] active:scale-95 ${
              sortMode === value
                ? "bg-charcoal text-white shadow-card"
                : "bg-panel text-inkSoft shadow-sm ring-1 ring-black/5 hover:ring-black/10"
            }`}
          ...
          {sortedItems.map(({ group, product }, idx) => {
            ...
            return (
              <Link key={group.id} to={`/product/${product.id}`} className="block">
                <Card
                  className={`animate-card-in grid gap-3 p-3 transition-shadow hover:shadow-lift sm:grid-cols-[86px_1fr_auto] sm:items-center ${
                    isHot ? "border-coral/30 bg-coral/5 ring-1 ring-coral/30" : ""
                  }`}
                  style={{ animationDelay: `${idx * 50}ms` }}
                >
                  <div className="relative h-24 w-24 overflow-hidden rounded-xl bg-panelSoft sm:h-[86px] sm:w-[86px]">
                    <div className={`absolute left-1 top-1 z-10 rounded-full px-1.5 py-0.5 text-[10px] font-bold text-white ${expired ? "bg-inkSoft" : "bg-charcoal"}`}>
                  ...
                  <div className="min-w-0 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="line-clamp-1 text-base font-bold text-ink">
                        {product.name}
                      </h2>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-inkSoft">
                  ...
                  <div className="grid gap-1.5 text-left sm:text-right">
                    ...
                    <p className="text-sm font-bold text-mint">-{pct}%</p>
