    ...
    <div className="fixed bottom-20 left-3 z-50 flex flex-col gap-2 md:bottom-6" aria-live="polite">
      {toasts.map((toast) => {
        ...
        return (
          <div
            key={toast.id}
            className="animate-slide-in-left flex max-w-[260px] items-center gap-3 rounded-xl border border-black/5 bg-white/96 px-3 py-2.5 shadow-xl backdrop-blur-sm"
          >
            <div className={cn("grid h-7 w-7 shrink-0 place-items-center rounded-full text-white", color)}>
              <Icon size={14} />
            </div>
